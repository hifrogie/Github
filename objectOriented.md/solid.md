...
- 의존성 이란 
    1. 함수에 필요한 클래스 또는 참조 변수나 객체에 의존하는 것
- 주입 이란?
    1. 그렇다면 위의 함수에 필요한 클래스 또는 참조변수나 객체 같은 것들은 어떻게 생성될까요?
    2. 직접 생성하거나 누군가 생성해준 것을 이용하는 방법이 있지만 이럴때 의존성 주입을 사용합니다.
    3. 보통은 내부에서 필요한 내용을(객체) 생성하여 참조/사용을 내부가 아니라 외부에서 객체를 생성해서 넣어주는 것을 주입한다고 한다.
    4. 그렇기 때문에 개발자들이 객체를 생성하는 번거로움과 다양한 테스트 케이스를 고려하는 경우를 줄이고 변수 사용과 개발에 더욱이 집중할 수 있게 해주며 클래스간의 결합도를 낮추어 의존성을 줄입니다.
- 의존성 주입이란?
    1. 코드에서 두 모듈 간의 연결을 의미함.
    2. 일반적으로 둘 중 하나가 다른 하나를 어떤 용도를 위해 사용함.
    3. 객체지향언어에서는 두 클래스 간의 관계라고도 말함.
    4. 클래스 간의 의존성이 줄어들면 유지보수시 매우 편함.
- 의존성 주입 장점
    1. Unit Test가 용이해짐
        - Unit Test : 컴퓨터 프로그래밍에서 소스 코드의 특정 모듈이 의도된 대로 정확히 작동하는지 검증하는 절차
    2. 코드의 재사용성을 높여준다.
    3. 리팩토링이 수월함.
    4. 객체 간의 의존성(종속성)을 줄이거나 없앨 수 있음.
    5. 객체 간의 결합도를 낮추면서 유연한 코드를 작성할 수 있음
    6. 보일러 플레이트 코드 감소
     - 보일러 플레이트 코드 : 최소한의 변경으로 여러곳에 재사용되며, 반복적으로 비슷한 형태를 띄는 코드
    7. 스코프를 이용한 객체 관리
- 의존성 주입의 단점
