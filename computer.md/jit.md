...
3. JIT를 쉽게 말하면 동적 번역(dynamic translation)이고 동적 번역은 실제 바이트 코드를 실행하는 시점에서 가상머신이 바이트 코드를 기계어로 변환함.
    - 가상 머신(JVM) : 자바 바이트 코드를 실행하는 실행기
...
- 캐시(cache) : 실제 메모리와 cpu사이에서 빠르게 전달을 위해서 미리 데이터들을 저장해두는 좀 더 빠른 메모리이다.
...
2. 어떤 예외를 다른 예외로 감싸는 것 이를 예외 연결(exception chaining)이라고 한다.
3.  언제 사용하나요?
    - 세부적인 사항을 포괄적인 사항으로 포함시킬때 사용
    - check 예외를 unchecked 예외로 변경시 사용(try-catch문 사용을 줄일 수 있음)
